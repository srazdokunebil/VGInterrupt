...
function VGI_Interrupt(unitName, spellName)
		...
		if ( not UnitIsPlayer( "target" ) and VGI_Spells[ targetName ] ~= nil )  then
			if ( VGI_EnemyCastBar.inProgress and VGI_EnemyCastBar.caster == targetName and ( VGI_EnemyCastBar.targetIconIndex == targetRaidIconIndex or VGI_EnemyCastBar.targetIconIndex == "0" ) ) then
				...
				if ( VGI_Spells[ targetName ][ VGI_EnemyCastBar.spellName ].isChanneled == true ) then
					VGI_Interrupt(VGI_EnemyCastBar.caster, VGI_EnemyCastBar.spellName);
				else -- else, delay it
					...
					if ( VGI_MobsCasting[ targetName ][ VGI_EnemyCastBar.spellName ].count >= 1 or alpha == 1 ) then VGI_Interrupt(VGI_EnemyCastBar.caster, VGI_EnemyCastBar.spellName); end
				end

				if VGI_EnemyCastBar.spellName and VGI_EnemyCastBar.caster then
					VGI_Interrupt(VGI_EnemyCastBar.caster, VGI_EnemyCastBar.spellName);
					return
				end
			...
			--VGI_Interrupt(targetName, "invalid");
		end
	end
end
...
function handleSpellCast( targetRaidIconIndex, targetName, spellName )
	...
	vr.log.Warn(">>> SPELLCAST <<<")
	vr.log.Warn(tostring(VGI_EnemyCastBar.caster).." is casting "..tostring(VGI_EnemyCastBar.spellName))
